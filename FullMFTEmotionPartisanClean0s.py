...
final_output_path = '/Users/mathewfeehan/Library/Mobile Documents/com~apple~CloudDocs/PhD Research/TestRussianTweetsUncleaneResults.csv'
cleaned_output_path = '/Users/mathewfeehan/Library/Mobile Documents/com~apple~CloudDocs/PhD Research/TestRussianTweetsCleaned.csv'
...
# Define MFT models and their corresponding output column names
models = {
...
def process_sentiment_and_engagement(chunk):
    """Compute sentiment score, engagement (log-transformed),
    partisan words, and affect frequencies using NRCLex."""
    ...
    # Engagement calculation: sum counts and apply log1p transform
    chunk['engagement'] = chunk[['quote_count', 'reply_count', 'like_count', 'retweet_count']].sum(axis=1).apply(np.log1p)
    
    # Partisan word detection (simple matching)
...
def classify_mft_gpu_all(chunk, models, batch_size=2048):
    """Perform GPU-based MFT classification on tweet_text for each model.
    This function uses the Hugging Face pipeline."""
    chunk = chunk.copy()
...
def process_and_classify_chunk(chunk, models):
    """Process a chunk of data: compute sentiment, engagement, engagement per follower,
    and run GPU-based classification. Returns a DataFrame with all original and new columns."""
    ...
    return chunk[original_columns + additional_columns]

def clean_mft_columns(file_to_clean, output_clean_file):
    """Load CSV file, replace entries in MFT columns containing 'not' with NaN, and save cleaned CSV."""
    data = pd.read_csv(file_to_clean)
    ...
    data.to_csv(output_clean_file, index=False)
    print(f"Cleaned data saved to {output_clean_file}")

# -------------------------------------------------------------------------------
# Process the CSV file in chunks and append results to final_output_path
...
# -------------------------------------------------------------------------------
# Clean the final output by removing entries with 'not' in MFT columns

clean_mft_columns(final_output_path, cleaned_output_path)
